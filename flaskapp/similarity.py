...
def stemAndLemmatize(data):
    wordnet_lemmatizer = WordNetLemmatizer()
    porter_stemmer=PorterStemmer()
    data=' '.join([ wordnet_lemmatizer.lemmatize(x) for x in data.split(' ')])
    #data=' '.join([ porter_stemmer.stem(x) for x in data.split(' ')])
    return(data)
...
class wordSimilarity():
    ...
    def removeUnnecessaryCharacter(self):
        self.fullString=self.fullString.replace(',',' ')
        self.fullString = re.sub('[^0-9a-zA-Z .]+', '', self.fullString)
    ...
    def getDistances(self):
        # Generate the dimensional array
        distanceData=[]
        for word in self.uniqueWords:
            distanceData.append(self.vectors[self.word2int[word]])
            
        # TSNE to convert to 2 D
        self.coordinateValues=TSNE(n_components=2).fit_transform(distanceData)
        self.distances=scipy.spatial.distance.squareform(scipy.spatial.distance.pdist(self.coordinateValues, 'euclidean')).tolist()
